...
/* Private variables ---------------------------------------------------------*/
uint8_t Bot_State;
uint8_t Bulk_Data_Buff[PACK_MAX_SIZE];  /* data buffer*/
uint16_t Data_Len;
Bulk_Only_CBW CBW;
Bulk_Only_CSW CSW;
...
void Mass_Storage_In (void)
{
    switch (Bot_State)
    {
        ...
        case BOT_ERROR:
          ...
          if (USB_GetEpRxSts(EP5_OUT) == EP_RX_STALL)
          {
...
void Mass_Storage_Out (void)
{
    ...
    Data_Len = USB_SilRead(EP5_OUT, Bulk_Data_Buff);
...
{
    USB_SilWrite(EP4_IN, Data_Pointer, Data_Len);
...
void Set_CSW (uint8_t CSW_Status, uint8_t Send_Permission)
{
    ...
    USB_SilWrite(EP4_IN, ((uint8_t *)& CSW), CSW_DATA_LENGTH);
