...
/* Header parser: "+GTRECV: <id>,<len>\n" */
static int parse_gtrecv_hdr(const char *buf, int len,
...
static int urc_gtrecv(at_urc_info_t *info)
{
    ...
    /* Fibocom appends CRLF after binary — request 2 trail bytes. */
    rc = at_urc_recv_split(info, parse_gtrecv_hdr, 2, &id, &payload, &plen);
    if (rc > 0) return rc;
    if (rc < 0) return 0;
...
static int work_sock_send(at_env_t *env)
{
    ...
    /* Fibocom replies "+GTCPSEND: <id>,OK" — match on ",OK" or bare "OK". */
