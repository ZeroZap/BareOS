...
/* Convenience aliases matching the simple xy_crc.h API */
static inline uint16_t xy_crc16(const uint8_t *data, uint32_t len)
{
    return xy_crc16_ccitt((uint8_t *)data, (uint16_t)len);
}
static inline uint32_t xy_crc32_simple(const uint8_t *data, uint32_t len)
{
    return xy_crc32_normal((uint8_t *)data, (uint16_t)len);
}
static inline uint8_t xy_crc8_simple(const uint8_t *data, uint32_t len)
{
    return xy_crc8_normal((uint8_t *)data, (uint16_t)len);
}
