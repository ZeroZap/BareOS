...
int32_t xy_vsnprintf(char *buf, uint32_t size, const char *fmt, va_list args)
{
    if (!buf || !fmt || size == 0) return -1;

    /* Write directly into the caller's buffer via xy_vsprintf, then clamp.
     * We use a module-level static scratch buffer (already exists: g_print_buf)
     * to avoid adding another 1 KB stack frame.                              */
    int32_t len = xy_vsprintf(g_print_buf, fmt, args);
    if (len < 0) return len;

    uint32_t copy_len = ((uint32_t)len < size) ? (uint32_t)len : size - 1u;
    if (copy_len > 0) {
        /* Use memcpy — strncpy pads with NUL which wastes cycles */
        int i;
        for (i = 0; i < (int)copy_len; i++) buf[i] = g_print_buf[i];
    }
    buf[copy_len] = '\0';
